...
public static class Extensions
{
    ...
    public static byte GetByteAtIndex(this ReadOnlySequence<byte> sequence, int index) => sequence.Slice(index).FirstSpan[0];

    public static async ValueTask<RecordHeader?> ReadHeaderAsync(this PipeReader reader, Endian endian, CancellationToken cancellationToken)
    {
        //read the record length
        var header = await reader.ReadAtLeastAsync(4, cancellationToken);
        //if we didn't read the full header, return null
        //TODO: differentiate between IsCanceled and IsComplete?
        if (header.Buffer.Length < 4)
        {
            return null;
        }
        //only get what we want to look at (to advance appropriately)
        var headerContent = header.Buffer.Slice(0, 4);

        //convert to ushort
        var length = headerContent.ReadStdfUShort(endian);
        //read the type
        var type = headerContent.GetByteAtIndex(2);

        //read the subType
        var subType = headerContent.GetByteAtIndex(3);
        reader.AdvanceTo(headerContent.End);
        return new RecordHeader(length, new RecordType((byte)type, (byte)subType));
    }

    public static Span<byte> CopyThrough(this ReadOnlySequence<byte> sequence, Span<byte> destination)
    {
        sequence.CopyTo(destination);
        return destination;
    }

    public static ushort ReadStdfUShort(this ReadOnlySequence<byte> sequence, Endian endian)
    {
        sequence = sequence.Slice(0, 2);
        var span = sequence.IsSingleSegment ? sequence.FirstSpan : sequence.CopyThrough(stackalloc byte[2]);
        return endian switch
        {
            Endian.Little => BinaryPrimitives.ReadUInt16LittleEndian(span),
            Endian.Big => BinaryPrimitives.ReadUInt16BigEndian(span),
